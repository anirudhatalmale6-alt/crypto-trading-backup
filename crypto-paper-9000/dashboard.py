    ...
    <div class="container">
        ...
        <div class="stats-grid" id="statsGrid">
            ...
            <div class="stat-card" style="border-color:#58a6ff">
                <div class="label" style="color:#58a6ff">TOTAL USDC (Used+Free)</div>
                <div class="value" id="statTotalUsdc" style="color:#58a6ff;font-size:16px">--</div>
            </div>
    ...
    <script>
        ...
        async function refresh() {
            try {
                ...
                var _usdcFree = data.usdc_balance || 0;
                var _usdcUsed = (data.portfolio ? data.portfolio.total_invested : 0) + (data.other_invested || 0);
                var _usdcTotal = _usdcFree + _usdcUsed;
                document.getElementById("statTotalUsdc").textContent = formatNumber(_usdcFree, 0) + " free + " + formatNumber(_usdcUsed, 0) + " used = " + formatNumber(_usdcTotal, 0) + " USDC";
...
def create_app(config: dict, database: Database, detector_ref: list) -> Flask:
    ...
    def api_data():
        ...
        if _now - _binance_cache["ts"] > _cache_ttl and not _binance_cache.get("fetching"):
            ...
            # Fetch full Binance portfolio in EUR
            _total_eur = 0
            _holdings = []
            try:
                _trader = detector_ref[0].trader if detector_ref else None
